...
(define-public (transfer-in (amount uint) (user-id uint) (asset-id uint) (asset-trait <sip010-trait>))
	(begin 
		...
		(print {type: "transfer-in", asset-id: asset-id, amount: amount, user-id: user-id, sender: tx-sender})
		(ok true)
	)
)
...
(define-public (request-transfer-out (amount uint) (user-id uint) (asset-id uint) (asset principal))
	(let 
		...
		(print {type: "request-transfer-out", request-id: request-id, user-id: user-id, asset-id: asset-id, amount: amount})
		(ok request-id)
	)
)
...
(define-public (approve-transfer-out (request-id uint) (approved bool))
	(begin
		...
		(print {type: "approve-transfer-out", request-id: request-id, approved: approved})
...
(define-public (transfer-out (request-id uint) (asset-trait <sip010-trait>))
	(let 
		...
		(print {type: "transfer-out", request-id: request-id, user-id: (get user-id request), asset-id: (get asset-id request), amount: (get amount request)})
		(ok true)		
	)
)	
...
(define-public (transfer (amount uint) (sender-id uint) (recipient-id uint) (asset-id uint))
	(let 
		...
		(print {type: "internal-transfer", asset-id: asset-id, amount: amount, sender-id: sender-id, recipient-id: recipient-id})
		(ok true)
	)
)
