		...
		(if (or (is-eq (get stop left-parent) u0) (is-order-triggered left-order-hash))
			...
			(let
				(
					...
					(is-buy (is-some (map-get? oracle-symbols (get taker-asset left-parent))))
					(symbol (try! (get-oracle-symbol-or-fail (if is-buy (get taker-asset left-parent) (get maker-asset left-parent)))))
				...
				(if (get risk left-parent) ;; it is risk-mgmt stop limit, i.e. buy on the way up (to hedge sell) or sell on the way down (to hedge buy)
					(begin
						(asserts! (if is-buy (>= (get value oracle-data) (get stop left-parent)) (<= (get value oracle-data) (get stop left-parent))) err-stop-not-triggered)
		...
		(if (or (is-eq (get stop right-parent) u0) (is-order-triggered right-order-hash))
			...
			(let
				(
					...
					(is-buy (is-some (map-get? oracle-symbols (get taker-asset right-parent))))
					(symbol (try! (get-oracle-symbol-or-fail (if is-buy (get taker-asset right-parent) (get maker-asset right-parent)))))
					(signer (try! (contract-call? .redstone-verify recover-signer (get timestamp oracle-data) (list {value: (get value oracle-data), symbol: symbol}) (get signature oracle-data))))
				)
				(asserts! (is-trusted-oracle signer) err-untrusted-oracle)
				...
				(if (get risk right-parent) ;; it is risk-mgmt stop limit, i.e. buy on the way up (to hedge sell) or sell on the way down (to hedge buy)
					(begin
						(asserts! (if is-buy (>= (get value oracle-data) (get stop right-parent)) (<= (get value oracle-data) (get stop right-parent))) err-stop-not-triggered)
				...
				(if is-buy 
					(let 
						(
							...
							(stop-floor-in-fixed (mul-down (div-down linked-limit-in-fixed parent-limit-in-fixed) (+ ONE_8 (get haircut-in-fixed risk-params_))))
							(linked-cap-in-fixed (mul-down parent-limit-in-fixed (- ONE_8 (div-down ONE_8 (* (get max-leverage risk-params_) ONE_8)))))
						) 
						(asserts! (>= (get stop left-linked) linked-limit-in-fixed) (err u111)) ;;err-invalid-stop-price)
						(asserts! (>= (get stop left-linked) stop-floor-in-fixed) (err u222)) ;;err-invalid-stop-price)
					...
					(let 
						(
							...
							(stop-cap-in-fixed (mul-down (div-down linked-limit-in-fixed parent-limit-in-fixed) (- ONE_8 (get haircut-in-fixed risk-params_))))
							(linked-floor-in-fixed (mul-down parent-limit-in-fixed (+ ONE_8 (div-down ONE_8 (* (get max-leverage risk-params_) ONE_8)))))
						) 
						(asserts! (<= (get stop left-linked) linked-limit-in-fixed) (err u333)) ;;err-invalid-stop-price)
						(asserts! (<= (get stop left-linked) stop-cap-in-fixed) (err u444)) ;;err-invalid-stop-price)
			...
			(let 
				...
				(if is-buy 
					(let 
						...
						(asserts! (>= (get stop right-linked) stop-floor-in-fixed) err-invalid-stop-price)
						(asserts! (<= linked-limit-in-fixed linked-cap-in-fixed) err-invalid-limit-price)
					)
					(let 
						(
						...
						(asserts! (<= (get stop right-linked) linked-limit-in-fixed) err-invalid-stop-price)
						(asserts! (<= (get stop right-linked) stop-cap-in-fixed) (err stop-cap-in-fixed)) ;;err-invalid-stop-price)
						(asserts! (>= linked-limit-in-fixed linked-floor-in-fixed) err-invalid-limit-price)
					)					
				)				
			)
