...
(define-private (transfer-in (amount uint) (user-id uint) (asset-id uint) (asset principal))
    (if (is-eq asset .token-wxusd)
        (contract-call? .stxdx-wallet-zero transfer-in amount user-id asset-id .token-wxusd)
...
(define-public (register-and-deposit (maker-pubkey (buff 33)) (amounts (list 10 uint)) (asset-ids (list 10 uint)) (assets (list 10 principal)))
    (transfer-in-many (try! (contract-call? .stxdx-registry register-user maker-pubkey)) amounts asset-ids assets)
)