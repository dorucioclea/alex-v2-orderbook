...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    // console.log(left-order, right-order);
    ...
    const response = chain.callReadOnlyFn(
      contractNames.exchange,
      'validate-match',
      [left-order, right-order, left-signature, right-signature, types.none()],
      sender.address,
    );
    const response-tuple = response.result.expectOk().expectTuple();
    assertEquals(response-tuple, {
